...
module.exports = {
  ...
  execute: async (bot, msg, args) => {
    ...
    await sendAnimeVideo(bot, msg.chat.id, msg.message_id);
  },
  ...
  onCallbackQuery: async function (bot, query) {
    try {
      ...
      if (data === "anime_next") {
        ...
        await bot.answerCallbackQuery(query.id, { text: "⏳ পরবর্তী অ্যানিমে ভিডিও পাঠানো হচ্ছে..." });
        ...
        writer.on("finish", async () => {
          ...
          await bot.sendVideo(chatId, fs.createReadStream(cachePath), {
            ...
            reply_markup: {
              inline_keyboard: [
                [
                  { text: "⏭️ 𝐍𝐄𝐗𝐓", callback_data: "anime_next" },
      ...
      console.error("Anime Callback Error:", err.message);
    }
  }
};

async function sendAnimeVideo(bot, chatId, messageId) {
  const link = links[Math.floor(Math.random() * links.length)];
  const caption = captions[Math.floor(Math.random() * captions.length)];
  const cachePath = path.join(__dirname, "cache", `anime_${Date.now()}.mp4`);
