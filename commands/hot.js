...
module.exports = {
  ...
  execute: async (bot, msg, args) => {
    ...
    const filePath = path.join(cacheDir, `funvideo_${Date.now()}.mp4`);

    const waitingMsg = await bot.sendMessage(chatId, "⏳ **Loading video, please wait...** 📥", {
    ...
    try {
      ...
      writer.on("finish", async () => {
        ...
        await bot.sendVideo(chatId, fs.createReadStream(filePath), {
          caption: "এই বুকাচুদা ! এতো হেন্ডেল মারলে হবে?🤬",
          reply_to_message_id: messageId
        });
      ...
      writer.on("error", async () => {
        ...
        await bot.sendMessage(chatId, "❌ **Failed to download video!** ⚠️", { 
          reply_to_message_id: messageId,
          parse_mode: "Markdown"
        });
      });
      ...
      if (fs.existsSync(filePath)) fs.unlinkSync(filePath);
      await bot.sendMessage(chatId, "❌ **VIDEO LOAD KORTE BYRTHO HOYACHE!** ⚠️", { 
        reply_to_message_id: messageId,
        parse_mode: "Markdown"
      });
    }
