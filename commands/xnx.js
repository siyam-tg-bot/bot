...
module.exports = {
    ...
    execute: async (bot, msg, args) => {
        ...
        try {
            const configRes = await axios.get(nix, { timeout: 10000 });
            const base = configRes.data?.api;
            if (!base) throw new Error("API base not found");

            const res = await axios.get(`${base}/xnx?q=${encodeURIComponent(query)}`, { timeout: 15000 });
...
🖌️ 𝙲𝚁𝙴𝙰𝚃𝙴𝙳 𝙱𝚈: 𝐒𝐈𝐘𝐀𝐌-𝐇𝐀𝐒𝐀𝐍`;
            ...
            return bot.sendMessage(chatId, `❌ 𝙺𝙰𝙹 𝙺𝙾𝚁𝚃𝙴 𝚂𝙾𝙼𝙾𝚂𝚂𝙰 𝙷𝙾𝚈𝙴𝙲𝙷𝙴, 𝙰𝙱𝙰𝚁 𝙲𝙷𝙴𝚂𝚃𝙰 𝙺𝙾𝚁𝚄𝙽!`, {
                reply_to_message_id: messageId
            });
        }
    }
...
module.exports.handleCallbackQuery = async (bot, query) => {
    ...
    if (!data || !data.startsWith("xnx_")) return;
...
🖌️ 𝙲𝚁𝙴𝙰𝚃𝙴𝙳 𝙱𝚈: 𝐒𝐈𝐘𝐀𝐌-𝐇𝐀𝐒𝐀𝐍`;
    ...
    if (data.startsWith("xnx_dl_")) {
        ...
        if (!link) {
            return bot.answerCallbackQuery(query.id, { text: "𝙻𝙸𝙽𝙺 𝙿𝙰𝚆𝙰 𝙶𝙴𝙻𝙾 𝙽𝙰!", show_alert: true });
        }

        await bot.answerCallbackQuery(query.id, { text: "📥 𝙳𝙾𝚆𝙽𝙻𝙾𝙰𝙳𝙸𝙽𝙶 𝚅𝙸𝙳𝙴𝙾... 𝙿𝙻𝙴𝙰𝚂𝙴 𝚆𝙰𝙸𝚃." });

        try {
            const dlRes = await axios.get(`${base}/xnxdl?url=${encodeURIComponent(link)}`, { timeout: 20000 });
            const resData = dlRes.data?.result;
            
            if (!resData || !resData.files) {
                throw new Error("Download links not found");
            }
            ...
            const vidData = await axios.get(videoUrl, { responseType: "arraybuffer", timeout: 30000 });
            fs.writeFileSync(filePath, Buffer.from(vidData.data));

            const caption = 
`━━━━━━━━━━━━━━━━━━
🎬 𝚃𝙸𝚃𝙻𝙴: ${resData.title || 'N/A'}
            ...
            const inlineKeyboard = {
                reply_markup: {
                    inline_keyboard: [
                        [
                            { text: "𝙾𝚆𝙽𝙴𝚁", url: "https://t.me/ri_siyam" },
                            { text: "𝙰𝙳ⴷ 𝙱𝙾𝚃", url: "https://t.me/SiyamTgBot?startgroup=true" }
                        ]
                    ]
                }
            };
